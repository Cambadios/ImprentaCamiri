 ...
 */
export default function TopProductsBar({ qs, limit = 10, height = 520 }) {
  ...
  const rows = useMemo(() => {
    return [...rowsRaw].sort((a, b) => (b?.ingresos || 0) - (a?.ingresos || 0));
  }, [rowsRaw]);

  // Si hay muchos productos, usa barras horizontales
  const indexAxis = rows.length > 10 ? 'y' : 'x';

  const labels = useMemo(() => rows.map(r => r?.nombre || '—'), [rows]);
  const ingresos = useMemo(() => rows.map(r => Number(r?.ingresos || 0)), [rows]);
  const unidades = useMemo(() => rows.map(r => Number(r?.unidades || 0)), [rows]);
  ...
  const header = (
    <div className="flex items-center justify-between gap-2">
      <span className="text-lg font-semibold">Top {limit} productos por ingresos</span>
      <Button
        rounded
        text
        severity="secondary"
        ...
        onClick={() => {
          ...
          a.download = `top-productos_${new Date().toISOString().slice(0,10)}.png`;
          a.click();
        }}
      />
    </div>
