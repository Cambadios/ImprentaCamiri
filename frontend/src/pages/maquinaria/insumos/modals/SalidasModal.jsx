// src/pages/maquinaria/insumos/modals/SalidasModal.jsx
...
/**
 * Modal para listar movimientos de tipo SALIDA (auditoría).
 * Props:
 *  - open: boolean
 *  - onClose: () => void
 */
export default function SalidasModal({ open, onClose }) {
  ...
  const onPageChange = (e) => {
    const nextPage = (e.page ?? 0) + 1; // PrimeReact es 0-indexed
    setPage(nextPage);
  ...
  const referenciaBody = (row) => {
    if (row.referencia?.startsWith("PEDIDO:")) {
      const id = row.referencia.split(":")[1];
      return <span className="text-blue-600 font-semibold">Pedido {id}</span>;
    }
  ...
  return (
    <Dialog
      header="Salidas de Inventario (Maquinaria)"
      visible={open}
          ...
          <div className="col-span-1">
            ...
            <span className="p-input-icon-left w-full">
              ...
              <InputText
                ...
                placeholder="Motivo / Referencia (p.ej. PEDIDO: )"
                className="w-full"
              />
            </span>
          </div>
